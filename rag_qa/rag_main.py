...
def main(query_mode=True, directory_path="data"):
    ...
    if not query_mode:
        ...
        while True:
            ...
            source_filter_input = input(f"请输入制度类型 ({'/'.join(valid_sources)}) (直接回车默认不过滤)：").strip()
            ...
            if source_filter_input:
                if source_filter_input in valid_sources:
                    ...
                    logger.info(f"用户选择了制度类型过滤: {source_filter}")
                else:
                    logger.warning(
                        f"无效的制度类型 '{source_filter_input}'，将不过滤"
                    )
                    print(f"提示：输入的制度类型 '{source_filter_input}' 无效，将不过滤。")
...
if __name__ == '__main__':
    # main(query_mode=False, directory_path="rag_qa/data/enterprise_policies")
    # 或者通过命令行参数控制
    import argparse
    parser = argparse.ArgumentParser(description="企业制度问答系统入口")
