...
class VectorStore:
    ...
    def __init__(self,
        ...
        # todo 初始化 BGE-Reranker 模型，用于重排序检索结果
    ...
    def add_documents(self, documents):
        ...
        for i, doc in enumerate(documents):
            ...
            data.append({
                ...
                "policy_type": doc.metadata.get("policy_type", doc.metadata.get("source", "unknown")),
    ...
    def hybrid_search_with_rerank(self, query, k=conf.RETRIEVAL_K, source_filter=None):
        ...
        # todo  创建加权排序器，稀疏向量权重 0.7，稠密向量权重 1.0
        ...
        results = self.client.hybrid_search(
            ...
            ranker=ranker,
            limit=k,
            output_fields=["text", "parent_id", "parent_content", "source", "timestamp", "policy_type", "version", "status", "effective_date", "file_path"]
        )[0]
        ...
        if parent_docs:
            ...
            # todo 使用 BGE-Reranker 计算每个配对的得分
...
if __name__ == "__main__":
    ...
    # directory_path = 'rag_qa/data/enterprise_policies'
    # print(f"embedding_function.dim--》{vector_store.embedding_function.dim}")
    # documents = process_documents(directory_path)
    # vector_store.add_documents(documents)
    query = "调休申请需要哪些材料"
    results = vector_store.hybrid_search_with_rerank(query, source_filter='考勤休假')
    print(f'results-->{results}')
    print(f'results-->{len(results)}')
