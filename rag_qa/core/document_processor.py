...
def load_documents_from_directory(directory_path):
    ...
    # 从目录名提取企业知识域（如 "hr_data" -> "hr"）
    # print(f'1---》{os.path.basename(directory_path)}')
    # print(f'source-->{source}')
    # 遍历指定目录及其子目录
    for root, _, files in os.walk(directory_path):
        # Allow a root directory such as enterprise_policies/考勤休假 to become
        # the Milvus source filter instead of assigning every document one label.
        source = os.path.basename(root).replace("_data", "")
        # print(f'root---》{root}')
        # print(f'files---》{files}')
        # 遍历当前目录下的所有文件
        for file in files:
            ...
            if file_extension in supported_extensions:
                ...
                try:
                    ...
                    for doc in loaded_docs:
                        ...
                        doc.metadata.setdefault("policy_type", source)
                        doc.metadata.setdefault("version", "v1.0")
                        doc.metadata.setdefault("status", "active")
                        doc.metadata.setdefault("effective_date", datetime.now().date().isoformat())
                        # 为文档添加文件路径元数据
    ...
    for i, doc in enumerate(documents):
        ...
        for j, parent_doc in enumerate(parent_docs):
            ...
            for k, sub_chunk in enumerate(sub_chunks):
                ...
                # print(f'的sub_修改后chunk--》{sub_chunk}')
                # 将子块添加到子块列表中
...
if __name__ == '__main__':
    directory_path = os.path.join(rag_qa_path, 'data', 'enterprise_policies')
    # documents = load_documents_from_directory(directory_path)
    # print(documents)
