...
class RAGSystem:
    ...
    def generate_answer(self, query, source_filter=None, history=None):
        ...
        logger.info(f"开始处理查询: '{query}', 知识域过滤: {source_filter}")
        #todo 下文为添加内容 验证历史
...
#todo 上文为添加内容
        ...
        # 公共信息直接调用 LLM；企业制度问题才进入 RAG 检索。
        if query_category == "公共信息查询":
            logger.info("查询为公共信息，直接调用 LLM")
            context = ""
        else:
            logger.info("查询为企业制度咨询，执行 RAG 流程")
            strategy = self.strategy_selector.select_strategy(query)
...
if __name__ == '__main__':
    ...
    answer = rag_system.generate_answer(query="调休申请需要哪些材料", source_filter="考勤休假")
    for vlaue in answer:
        print(vlaue)
    # rag_system._retrieve_with_subqueries(query="年假和调休的区别是什么？", source_filter="考勤休假")
    # result = rag_system._retrieve_with_hyde(query="费用报销需要哪些审批节点", source_filter="财务审批")
    # print(result)
    # print(len(result))
