...
def stream_query(query: str, source_filter: str = None, session_id: str = None):
    ...
    data = {
        "query": query,                # 用户的问题
        "source_filter": source_filter, # 制度类型过滤条件（可选）
...
if __name__ == "__main__":
    ...
    #
    # # 示例 2: 带制度类型过滤的查询
    stream_query("年假申请需要哪些材料", source_filter="考勤休假")
    #
    # print("\n" + "-" * 50 + "\n")
    #
    ...
    # print("🔄 模拟多轮对话（同一会话ID）")
    # stream_query("新员工入职需要办理哪些事项？", session_id=session)
    # # print("\n" + "-" * 30)
    # stream_query("那转正手续呢？", session_id=session, source_filter="人事制度")